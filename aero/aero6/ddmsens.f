...
C *** CALCULATE DGAMA ***
      ...
      IF (CC.EQ.'A'.OR.CC.EQ.'B'.OR.CC.EQ.'C'.OR.
     &    CC.EQ.'D'.OR.CC.EQ.'E'.OR.CC.EQ.'F'.OR.
     &    CC.EQ.'G'.OR.CC.EQ.'H'.OR.CC.EQ.'I'.OR.
     &    CC.EQ.'J') THEN
        CALL DELGAMA1( DGAMA )
      ELSE
         CALL DELGAMA2( DGAMA,frow )
      ENDIF

c     DGAMA = 0.0D0 ! set to ignore activity
...
C *** ADJUST FOR MINOR SPECIES ***
      ...
      DO I = 1,NPAIR
         SGAMA(I) = 0.D0
         DO J = 1,NIONSPC
            SGAMA(I) = SGAMA(I)+DGAMA(J,I)*SENS(J)
         ENDDO
      ENDDO

c     SGAMA = 0.0D0 ! set to ignore activity
