...
The atmospheric component of the Model for Prediction Across Scales (MPAS) is developed by the National Center for Atmospheric Research (NCAR). This meteorological model can be coupled with CMAQ version 5.5. MPAS-CMAQ is currently available in a pre-configured simulation that uses a globally-uniform 120 km resolution mesh and the CRACMM2 chemical mechanism. 

The MPAS-CMAQ model described here cannot be run in a stand-alone mode, with meteorological fields generated by MPAS used as input to CMAQ. 
