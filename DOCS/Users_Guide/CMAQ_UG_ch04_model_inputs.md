...
See the [CMAQ Data](../CMAQ_Data.md) page for a CMAQ-ready input files for multiple domains and simulation years.
...
CMAQ can use boundary conditions derived from global chemistry models (GCMs). While BCON does not directly support processing of datasets from GCMs (other than the hemispheric version of CMAQ) in their native formats, users could develop their own custom codes (eg., [aqmbc](https://github.com/barronh/aqmbc)) to transform other GCM datasets into I/O API format, which would then allow these datasets to be input into BCON in the same way as an existing CCTM output file.
...
CMAQ requires a basic set of input files: initial condition file, which is created by [ICON process](https://github.com/USEPA/CMAQ/blob/main/PREP/icon/) or previous day output; boundary condition file, which is created by [BCON process](https://github.com/USEPA/CMAQ/blob/main/PREP/bcon/); emission files; and meteorological data created by [MCIP](https://github.com/USEPA/CMAQ/tree/main/PREP/mcip) using WRF and terrain data. Additional input files may be required based on specific run time options. CMAQ output files include a basic set of files with aerosol and gas-phase species concentrations, wet and dry deposition estimates, and visibility metrics, and an auxiliary set of output files for diagnosing model performance and in-line-calculated emissions.  Model outputs are discussed in [Chapter 7](CMAQ_UG_ch07_model_outputs.md).
...
|[CMAQ_CTRL_NML](#cmaqctrl) <a id=cmaqctrl_t></a>|ASCII|n/a|n/a|CMAQ repo|required|
|[CMAQ_CH_CTRL_NML](#chemctrl) <a id=chemctrl_t></a>|ASCII|n/a|n/a|CMAQ repo|required|
...
**CMAQ_Control.nml: Control of ELMO and DESID Output File Contents and Select Input Variables**
<!-- BEGIN COMMENT -->
[Return to Table 4-1](#cmaqctrl_t)
<!-- END COMMENT -->
...
their budget calculated.   

<a id=chemctrl></a>
**CMAQ_Chem_Control_${MECH}.nml: Control of Chemical Mechanism-Dependent Specifications for DESID and ELMO**

<!-- BEGIN COMMENT -->

[Return to Table 4-1](#chemctrl_t)
...
number of chemical families that will be specified (N_Chem_Fams) and the maximum number of chemical species among all defined chemical families (Max_Chem_Fam_Members). 
For example, if a user defined three chemical families including 'NOX','AROMATICS', and 'BSOA', then a chemical family entry could be written as follows:
```
&Chem_FamVars  
  N_Chem_Fams = 3  
...
The final section of the Chemical Control Namelist contains an extensive list of predefined ELMO composite variables. These may be used directly as members of File_Vars 
in the CMAQ Control Namelist to add each composite to an output file. They may also be used as members of ELMO Keywords, or used in the definition of another ELMO composite. 
Please see the Appendix on ELMOv2 for detailed descriptions:

* [Jump to ELMOv2 Description](Appendix/CMAQ_UG_appendixF_elmo_output.md) in Appendix F.
* [Jump to ELMOv2 Tutorial](Tutorials/CMAQ_UG_tutorial_ELMO.md) for step by step instructions on adding custom diagnostic variables to the ELMO system.
