...
A full set of inputs for 2022 are provided for the 12US1 domain (299 column x  459 row x 35 layer, 12-km horizontal grid spacing) on Amazon Web Services (AWS) Open Data Registry, including emissions compatible with both the CRACMM2 and CRACMM3 chemical mechanisms.  Input files can be used for running CMAQv5.5 (with CRACMM2) or CMAQv6.0 (with CRACMM2 or CRACMM3). Sample scripts and control files are provided that are specific to this emissions case.  2022 CRACMM2 emissions largely follow the [EPA's 2022 modeling platform inventory](https://www.epa.gov/air-emissions-modeling/2022v1-emissions-modeling-platform) methods used to generate the data available from this AWS S3 bucket except for CRACMM2 speciation, which follows [Skipper et al., 2024](https://doi.org/10.5194/acp-24-12903-2024), and mobile emissions have been further updated to follow [Murphy et al. 2023](https://doi.org/10.5194/acp-23-13469-2023). 

Note that the 12US1 inputs are  netCDF-4/HDF5 compressed files to substantially reduce file sizes. Through testing at the EPA, we’ve noticed that certain domains encounter model crashes from reading in large amounts of compressed netCDF data.  A work around for those cases is uncompressing the data manually via [nccopy 1](https://www.unidata.ucar.edu/software/netcdf/workshops/2011/utilities/Nccopy.html) or [m3cple](https://www.cmascenter.org/ioapi/documentation/all_versions/html/M3CPLE.html) (compiled with HDF5) before running the CMAQ simulation.
