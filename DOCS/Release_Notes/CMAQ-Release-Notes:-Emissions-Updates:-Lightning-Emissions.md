...
- Added more options for using satellite GLM flashes/energy and the synergized methodology with WWLLN data. These options have been tested with the CMAQv6.0 platform and the preliminary evaluation resutls are available in AMET. The default option is the same as before and using it will not impact mode results.
