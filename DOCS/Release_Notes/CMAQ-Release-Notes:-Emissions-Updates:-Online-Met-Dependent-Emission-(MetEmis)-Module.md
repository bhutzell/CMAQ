...
In order to use the MemEmis feature, a compatible emission file must be produced from SMOKE. This file must have emission rates tabled as a function of ambient temperature, which is then interpolated by CMAQ. In the Run script example below, the file 'emis_mole_onroad.ncf' is an example of this file with MetEmis compatibility. See the SMOKE User GUide for instructions on how to output such a file.  
