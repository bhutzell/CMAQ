# Frequently Asked Questions for Using the CMAQv6.0 Version

## Table of Contents:
* [Do I need to update from v5.5 to v6.0?](#update_v55_v60b)
* [What do I need to do to update from v5.5 to v6.0?](#update_v55_v60b)
...
* [What differences should I expect in my model results with v6.0 compared to v5.5?](#diff_v55_v60_model_results)
...
## Do I need to update from v5.5 to v6.0?
CMAQv6.0 contains advances in both scientific and user-support algorithms. In many applications the model results will be consistent between versions 5.5 and 6.0. 
...
- **Updates to the Decoupled Direct Method in Three Dimensions (CMAQ-DDM3D)**  
 - **Address DDM3D instability in sensitivity fields**  
  Fixes DDM3D instability in sensitivity fields after heterogeneous chemistry due to inconsistent unit conversions. [Release Note](../Release_Notes/CMAQ-Release-Notes:-Instrumented-Models:-CMAQ-DDM3D.md)
...
CMAQv6.0 introduces CRACMM version 3. CRACMM3 includes several updates to CRACMM2. These updates improve the representation of gas-phase and aerosol chemistry in multiple environments. In addition to the base CRACMM3 mechanism, CMAQv6.0 includes CRACMM3M, with extended marine chemistry, and CRACMM3HAPS which includes additional Hazardous Air Pollutants. If you are interested in learning more, please see the CRACMM3 release notes:
 ...
 Peroxy radical products from ozonolysis of monoterpenes and from photolysis of monoterpene aldehydes were updated to species that promptly form highly oxygenated organic molecules (HOM). These changes increase organic aerosol concentrations, particularly in regions with high biogenic emissions. [Release Note](../Release_Notes/CMAQ-Release-Notes:-Chemistry:-Community-Regional-Atmospheric-Chemistry-Multiphase-Mechanism-(CRACMM).md#peroxy-radical-products-from-monoterpene-ozonolysis-and-monoterpene-aldehyde-photolysis)
 ...
 Expands CRACMM3 to create a new mechanism ideally suited for domains with large marine environments due to detailed halogen chemistry. [Release Note](../Release_Notes/CMAQ-Release-Notes:-Chemistry:-Community-Regional-Atmospheric-Chemistry-Multiphase-Mechanism-(CRACMM).md#halogen-chemistry-in-cracmm3m)
...
Revises model default from `Budget_Diag = .TRUE.` to `Budget_Diag = .FALSE.`. This change turns off the budget diagnostic tool reducing runtime by 10%. [Release Note](../Release_Notes/CMAQ-Release-Notes:-Diagnostic-Options.md#changes-in-henrys-law-computation-and-budget-tool)
...
<a id=update_v55_v60b></a>
## What do I need to do to update from v5.5 to v6.0?
...
* If you have already migrated to v5.5, you will not need any additional input files to run with the analogous options in v6.0 . CRACMM2 emission inputs can be used to run CMAQv6.0 with the new CRACMM3 mechanism without any modifications. 
...
* CMAQv6.0 updates to ELMOv2 replace ELMOv1.0 diagnostic output files. The CCTM defaults now produce the CCTM_ELMO1 which supersedes older CCTM_ELMO files while adding additional gas and particle phase diagnostic aggregates not available in ELMOv1.0. ELMOv2.1 also produces a CCTM_ELMO2_DEP file which includes gas and particle phase dry and wet deposition hourly aggregates.
 
* CMAQv6.0 updates two of the three photolysis diagnostic files (CCTM_PHOTDIAG1 and CCTM_PHOTDIAG3). The CCTM_PHOTDIAG1 file remains largely unchanged, with the only changes being in the diagnostic variables: AOD_W550_ANGST (Aerosol Optical Depth at 550 nm based on an Angstrom Interpolation) and AAOD_W550_ANGST (Aerosol Absorption Optical Depth at 550 nm based on an Angstrom Interpolation). The updates now enable calculation of AOD_550 values when the sun is below the horizon. The CCTM_PHOTDIAG3 file changed in three different ways. (1) The variable representing total extinction, the sum of absorption and scattering at various wavelengths from gas, aerosols and clouds, is no longer reported; this variable is now replaced by the cloud extinction; the total extinction can be computed manually as the sum of gas, aerosol and cloud extinction. (2) The photolysis diagnostic variable names "EXT_AERO_W" are changed to "AERO_EXT_W", making the naming convention consistent with how the gas is reported. (3) The photolysis diagnostics for AOD_550 and aerosol asymmetry and extinction are now available at all simulation hours, independent of the position of the sun. [Release Note #1](../Release_Notes/CMAQ-Release-Notes:-Chemistry:-Photolysis.md#remove-uninitialized-variable-and-correct-a-diagnostic-in-cctms-inline-module-for-photolysis-frequencies), [Release Note #2](../Release_Notes/CMAQ-Release-Notes:-Photolysis.md#remove-uninitialized-variable-and-correct-a-diagnostic-in-cctms-inline-module-for-photolysis-frequencies)

<a id=diff_v55_v60_model_results></a>
## What differences should I expect in my model results with v6.0 compared to v5.5?
The following summary is based on our testing of CMAQv5.5 and CMAQv6.0 using annual 2022 simulations performed over the northern hemisphere (108 km horizontal grid spacing, 44 vertical layers) and the CONUS (both at 12 km and 36km horizontal grid spacing and using 35 vertical layers). The 108 km simulations were used to generate lateral boundary conditions for the 12 km and 36 km simulations and were also analyzed separately to quantify the impacts of model changes on large-scale atmospheric composition. All simulations had the inline windblown dust option turned on.

The CMAQv5.5 108 km simulation used the CB6R5M_AE7_AQ chemical mechanism that includes a detailed representation of halogen chemistry while the 36 km and 12 km simulations used the CRACMM2 chemical mechanism that includes a condensed representation of halogen chemistry. Boundary conditions for the CMAQv5.5 36 km and 12 km simulations were mapped from CB6R5M_AE7_AQ to CRACMM2.

The CMAQv6.0 108 km simulation used the CRACMM3M chemical mechanism that includes a detailed representation of halogen chemistry while the 36 km and 12 km simulations used the CRACMM3 chemical mechanism that includes a revised condensed representation of halogen chemistry. Both the CMAQv6.0 CRACMM3M and CRACMM3 chemical mechanisms include photolysis of aerosol nitrate as described for hemispheric applications in [Sarwar et al. (2024)](https://doi.org/10.1016/j.scitotenv.2024.170406) and CONUS applications in [Sarwar et al. (2025)](https://doi.org/10.1016/j.scitotenv.2025.178968).

### Ozone
* v6.0 vs. v5.5 hemispheric comparisons: ozone mixing ratios generally show substantial increases in v6.0 throughout most of the northern hemisphere and throughout the year, with the largest increases of 5 – 15 ppbV occuring during springtime. These increases occur throughout the troposphere. While there are many differences between the CB6R5M_AE7_AQ chemical mechanism used for the CMAQv5.5 simulations and the CRACMM3M chemical mechanism used for the CMAQv6.0 simulations, the primary driver of these ozone increases is the addition of aerosol nitrate photolysis in CRACMM3M. The magnitude and spatial and seasonal patterns of the increase is consistent with [Sarwar et al. (2024)](https://doi.org/10.1016/j.scitotenv.2024.170406).
* v6.0 vs. v5.5 CONUS comparisons: consistent with the hemispheric simulations providing boundary conditions, ozone mixing ratios over the CONUS also show substantial increases in v6.0 , with the largest increases of 10 – 13 ppbV occuring during springtime over parts of the intermountain West. A v6.0 CONUS sensitivity simulation utilizing the CRACMM2 mechanism (which does not include the aerosol nitrate photolysis effect) instead of the CRACMM3 mechanism confirmed that the majority of this increase (75% or more) is caused by higher ozone from the boundary conditions. This is consistent with the results reported in [Sarwar et al. (2025)](https://doi.org/10.1016/j.scitotenv.2025.178968) who also reported that the majority of the impacts of aerosol nitrate photolysis on CONUS simulations is through its effect on large-scale tropospheric ozone.

### PM2.5
* Both the hemispheric and CONUS simulations show decreases in PM2.5 due to the reductions of windblown dust emissions caused by including the effects of non-photosynthetic vegetation (or brown vegetation) on these emissions in v6.0 . The CONUS simulations show a further decrease in these emissions due to a code correction when using the windblown dust option with meteorological fields configured with NLCD40 land use, the option used in our CONUS simulations.
...
The User's Guide chapters, tutorials, and appendices related to ELMOv2.1 and DESID have been updated for CMAQv6.0 . All other User's Guide content in this repository was last updated for the CMAQv5.5 release. Additional documentation updates will be included in the CMAQv6.0 release.

A full set of inputs for January 1 - December 31, 2022 are provided for the 12US1 domain, including CRACMM emissions that are compatible with both the CRACMM2 and CRACMM3. Input files can be used for running CMAQv5.5 (with CRACMM2) or CMAQv6.0 (with CRACMM2 or CRACMM3).
