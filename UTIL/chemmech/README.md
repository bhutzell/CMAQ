...
iv)	Using embedded comments in a species name cannot span two lines.

2)	Labels are used to define or refer to reactions, operators or other processes.

a)	Often start with "<" and ends with ">".

b)	Contain up to 16 non-blank characters long.

c)	Cannot contain a comment or a label delimiter.  

d)	A label may span lines.
