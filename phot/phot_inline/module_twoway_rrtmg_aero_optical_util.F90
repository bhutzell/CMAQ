...
! --------------------------------------------------------------------------
        type (complex_number) function c_add_rc (num, z1)

        type (complex_number), intent(in) :: z1
        real*8, intent(in)                :: num

        c_add_rc%real_part = z1%real_part + num
        c_add_rc%imag_part = z1%imag_part
...
private
public :: aero_optical, aero_optical2, aero_optical_CS

interface ghintBH 
  module procedure ghintBH_1, ghintBH_2
end interface
...
! ------------------------------------------------------------------
       subroutine ghintBH_CS (RCORE, RSHELL , XX, YY, xlnsig,  &                  
...
!      end if
       ...
       call BHCOAT_DW (XX,YY,RRFRL1,RRFRL2,QEXT,QSCA,QBACK,G_MIE, SUCCESS)
...
!          call BHCOAT_DW (XX,YY,RRFRL1,RRFRL2,QEXT,QSCA,QBACK,G_MIE)
!      end if
...
!    modified by Prof. Bruce T. Draine of Princetion University.
! 
...
! rather than arbitrary digits to increase accuracy 
       write (mystr, *) xx, yy, RRFRL1, RRFRL2
...
!            ancap  = ancap/(chix2*d1x2 - chipx2)
             ...
             ancap  = (refrel*d1x1 - d1x2) / ( (refrel*d1x1*chix2 - chipx2) * (chix2*d1x2 - chipx2) )
...
!         xback = xback +  (TWO_N_P_ONE) * (-1.)**n * (an-bn)
          factor = factor * (-1.0d0)
          xback = xback +  (TWO_N_P_ONE) * factor * (an-bn)
...
!    Now compute QQSCA,QQEXT,QBACK,and GSCA
       ...
       QQSCA = TWO * qsca * RYY
       QQEXT = TWO * qext * RYY
       QBACK = 0.5 *( xback * conjg(xback) ) * RYY
!      QBACK = REAL ( ( xback * conjg(xback) ) * RYY )
...
! ------------------------------------------------------------------
      SUBROUTINE BHCOAT_DW (XX, YY, RRFRL1, RRFRL2, QQEXT, QQSCA, QBACK, GGSCA, SUCCESS)
