...
! --------------------------------------------------------------------------------
        subroutine get_env_char (env_value, env_var, default_env_value, logdev)
          ...
          if ( loc_logdev .gt. 0 ) then
             ...
             if (default) then
                if (length .eq. 0) then
                   write( loc_logdev, '(5x,A21, 2x, A, 13x, a9)') env_var, '|', '(default)'
                else
                   write (myfmt, '(a22, i3.3, a9)') '(5x, A21, 2x, A, 2x, A', length, ', 1x, a9)'
                   write( loc_logdev, myfmt) env_var, '|', env_value, '(default)'
                end if
             else if (regular) then
                write (myfmt, '(a22, i3.3, a1)') '(5x, A21, 2x, A, 2x, A', length, ')'
